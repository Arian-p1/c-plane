...
func printBanner() {
	banner := `
 _   _           _                        _    _   _ ___
| \ | | _____  _| |_ _ __ __ _ _ __   ___| |_  | | | |_ _|
|  \| |/ _ \ \/ / __| '__/ _` + "`" + ` | '_ \ / _ \ __| | | | || |
| |\  |  __/>  <| |_| | | (_| | | | |  __/ |_  | |_| || |
|_| \_|\___/_/\_\__|_|  \__,_|_| |_|\___|\__|  \___/|___|
...
func printVersion() {
	fmt.Printf("GenieACS Gateway UI Service\n")
...
func printHelp() {
	fmt.Println("GenieACS Gateway UI Service")
	fmt.Println()
	fmt.Println("Usage:")
	fmt.Println("  ui [options]")
	fmt.Println()
	fmt.Println("Options:")
	...
	fmt.Println("  ui -config /etc/gateway/config.yaml")
	fmt.Println()
	fmt.Println("  # Start with configuration from environment variable")
	fmt.Println("  GATEWAY_CONFIG_PATH=/etc/gateway/config.yaml ui")
	fmt.Println()
	fmt.Println("Web UI will be available at:")
	fmt.Println("  http://localhost:8081 (or the configured port)")
}
